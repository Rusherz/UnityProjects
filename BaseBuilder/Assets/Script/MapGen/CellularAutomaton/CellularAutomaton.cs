...
namespace ProceduralToolkit.Examples
{
    ...
    public class CellularAutomaton
    {
        ...
        public class Config
        {
            ...
            public Ruleset ruleset = Ruleset.majority;
		...
		public void Simulate(string objectType)
        {
			...
			for (int x = 0; x < config.width; x++)
            {
				for (int y = 0; y < config.height; y++)
                {
                    ...
                    if (copy[x, y] == CellState.Dead)
                    {
                        ...
                        {
							World.currentWorld.PlaceFurniture (objectType, World.currentWorld.GetTileAt (x, y), false);
